...
class IrisRec:
    ...
    #     pts_norm = np.transpose(
    #         [(mt.cos((2 * mt.pi / wNorm[1]) * ang), mt.sin((2 * mt.pi / wNorm[1]) * ang)) for ang in range(wNorm[1])])
    #
...
class CASIAIris(IrisRec):
    ...
    _pathEye = os.environ['HOME'] + '/databases/CASIA-IrisV4-Interval'
    ...
    def getEyeImages(self, pathEye=_pathEye, pathMask=_pathMask, pathIris=_pathIris, pathNorm=_pathNorm):
        ...
        subjects_paths = [os.path.join(pathEye, d) for d in os.listdir(pathEye) if
                          os.path.isdir(os.path.join(pathEye, d))]
        ...
        #
            side_paths = [os.path.join(subject_paths, d) for d in os.listdir(subject_paths) if
                          os.path.isdir(os.path.join(subject_paths, d))]
            for e, side_path in enumerate(side_paths, start=1):
                ...
                # if pathMask and not os.path.exists(
                #         os.path.join(pathMask, os.path.split(subject_paths)[1], os.path.split(side_path)[1])):
                #     os.makedirs(os.path.join(pathMask, os.path.split(subject_paths)[1], os.path.split(side_path)[1]))
                # if pathIris and not os.path.exists(
                #         os.path.join(pathIris, os.path.split(subject_paths)[1], os.path.split(side_path)[1])):
                #     os.makedirs(os.path.join(pathIris, os.path.split(subject_paths)[1], os.path.split(side_path)[1]))
                # if pathNorm and not os.path.exists(
                #         os.path.join(pathNorm, os.path.split(subject_paths)[1], os.path.split(side_path)[1])):
                #     os.makedirs(os.path.join(pathNorm, os.path.split(subject_paths)[1], os.path.split(side_path)[1]))
        #
                eye_paths = [os.path.join(side_path, f) for f in os.listdir(side_path) if
                             f.endswith('.jpg') and os.path.isfile(os.path.join(side_path, f))]
        ...
        #             if pathMask:
        #                 imgpathMask = os.path.join(pathMask, os.path.split(subject_paths)[1],
        ...
        #             if pathNorm:
        #                 imgpathNorm = os.path.join(pathNorm, os.path.split(subject_paths)[1],
        #                                            os.path.split(side_path)[1], os.path.split(eye_path)[1])
    ...
    def SegIris(self, imgEye):
        ...
        pts_iris = np.transpose([(mt.cos((mt.pi / 180) * ang), mt.sin((mt.pi / 180) * ang)) for ang in range(360)])
        ...
        circles = cv2.HoughCircles(edges, cv2.HOUGH_GRADIENT, 3.1, 100, minRadius=10, maxRadius=100)
        tt = 0
        ...
        if radPupil is not None and len(circles) >= 1:
            ...
            imgIris[np.where(imgMask != 255)] = 255
        else:
...
class UBIRIS(IrisRec):
    ...
    def getEyeImages(self, pathEye=_pathEye, pathMask=_pathMask):
        ...
        imgEye_paths = [os.path.join(pathEye, f) for f in os.listdir(pathEye) if
                        f.startswith('C') and f.endswith('.tiff') and os.path.isfile(os.path.join(pathEye, f))]
